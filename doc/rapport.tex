...
N*N = Nombre de pièce
En d'autre terme l'algorithme va parcourir N*N fois la première case puis (N*N)*(N*N) la seconde case etc etc
...
Nous restons relativement satisfait de notre projet.

\section*{Annexe}

\lstinputlisting[inputpath=sources/, caption=Code sans Rotation (\lstname), label=codeComplet:NoRotation]{code.ml}

\lstinputlisting[inputpath=sources/, caption=Code avec Rotation (\lstname), label=codeComplet:Rotation]{codeRotation.ml}

\lstinputlisting[inputpath=sources/, caption=Code avec Rotation et première tuile Locked (\lstname), label=codeComplet:RotationTileLocked]{codeRotation1stTileLocked.ml}

\lstinputlisting[inputpath=sources/, caption=Prototype Python (\lstname), label=codeComplet:Prototype]{proto.py}
